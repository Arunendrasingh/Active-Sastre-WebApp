...
from main_app.models import Address, Femail_size_Chart, Oreder_Detail, Product_detail, Profile, MailSize, Product_Img, MyCart, Wishlist, user_feedback
...
def index(request):
    ...
    for product in get_product:
        ...
        total_price = productprice - \
            (int(productprice) * int(product.discount)) / 100
...
def place_order(request):
    if request.method == 'POST':
        ...
        if value[1] == 'female':
            ...
            size_detail = "Shoulder:-" + str(size_obj.shoulder) + ", Shoulder Full Lenght:-" + str(size_obj.shoulder_full_lenght) + ", Front Neck Depth:-" + str(size_obj.front_neck_depth) + ", Back Neck Depth:-" + str(size_obj.back_neck_depth) + ", Sleeve Length:-" + str(size_obj.sleeve_length) + ", Sleeve Around:-" + str(size_obj.sleev_around) + ", Armhole(around):-" + str(size_obj.arm_hole) + ", Dress Length:-" + str(size_obj.gown_length) + ", Neck Line:-" + str(size_obj.neck_line) + ", Upper Bust:-" + str(size_obj.upper_bust) + ", Chest/Bust:-" + str(size_obj.chest) + ", Stomach:-" + str(size_obj.stomach) + ", Hip:-" + str(size_obj.hips) + ", Thigh:-" + str(
                size_obj.thigh) + ", Chest Around:-" + str(size_obj.chest) + ", Waist:-" + str(size_obj.waist) + ", Blouse Length:-" + str(size_obj.blouse_length) + ", Knee:-" + str(size_obj.knee) + ", Calf:-" + str(size_obj.calf) + ", Ankel Hem:-" + str(size_obj.arm_hole) + ", Upper Chest:-" + str(size_obj.chest) + ", Neck to Solider:-" + str(size_obj.neck_to_solider) + ", Gown Length:-" + str(size_obj.gown_length) + ", Around Bust:-" + str(size_obj.around_bust) + ", upper_waist:-" + str(size_obj.upper_waist) + ", Waist to Ankel Length:-" + str(size_obj.waist_to_ankel_length) + ", Full Body Lenght:-" + str(size_obj.full_body_lenght)
        else:
            size_obj = MailSize.objects.filter(id=int(value[0]))[0]
            size_name = size_obj.name
            size_detail = "Paint Length:-" + str(size_obj.p_length) + ", Paint Waist:-" + str(size_obj.p_Waist) + ", Paint Hips:-" + str(size_obj.p_Hips) + ", Paint Thigh:-" + str(size_obj.p_Thigh) + ", Knee:-" + str(size_obj.p_Knee) + ", Leg Opening:-" + str(size_obj.p_Leg_Opening) + ", Crotch/Rise:-" + str(size_obj.p_Crotch_Or_Rise) + ", In-Seam:-" + str(size_obj.p_In_Seam) + ", Shirt Length:-" + str(size_obj.s_Shirt_lenght) + ", Sleeve Length:-" + str(
                size_obj.s_Sleeve_Length) + ", Shoulders:-" + str(size_obj.s_Shoulders) + ", Chest:-" + str(size_obj.s_Chest) + ", Overarm:-" + str(size_obj.s_Overarm) + ", Waistcoat Length:-" + str(size_obj.s_Waistcoat_Length) + ", Bicep(Loose):-" + str(size_obj.s_Bicep_Loose) + ", Chest:-" + str(size_obj.s_Front_Chest) + ", Stomach:-" + str(size_obj.s_Front_Stomach) + ", Front - Hip:-" + str(size_obj.s_Front_Hips) + ", Wrist:-" + str(size_obj.s_Wrist) + ", Neck:-" + str(size_obj.s_Neck)

        obj_order = Oreder_Detail(user_id=request.user.id, user_name=user_name, address=address, Product_Id=prod_id, pro_name=prod_name,
                                  Product_quantity=total_product,
                                  Pro_price=prod_price, size_id=int(value[0]), size_name=size_name, size_detail=size_detail,
                                  total_price=total_price, order_status='pending')
        obj_order.save()
        if obj_order.save:
            ...
            cart_obj.delete()
        messages.info(request, 'Your Order For Product' +
                      prod_name + 'has been placed successfully!')
...
def delete_from_wishlist(request, w_id):
    ...
    messages.info(
        request, 'Product is successfully! Deleted From your Wishlist')
    return redirect('/wishlist')
...
def wishlist(request):
    ...
    for wishlist1 in wishlist:
        ...
        prods.append({'product_detail': Product_detail.objects.filter(
            id=Product_id)[0], 'wishlist': wishlist1})

    return render(request, "wishlist.html", {'Product_Detail': prods, 'num_wish': num_wishlist})
...
def cart(request):
    ...
    return render(request, "cart.html", {"product": prods, 'ncart': n_cartprods, 'total_price': carttotal})
    ...
    if MyCart.objects.filter(product_id=pid, user_id = user_id).exists():
        ...
        for item in prods1:
            if item in prods1:
                ...
                item.save()
        messages.info(request, 'One more Product' +
                      pro_detail.name + ' added to your cart')
        return redirect("/cart")
    else:
        cart_obj = MyCart(user_id=user_id,
                          prod_amount=pro_amount, product_id=pid)
        cart_obj.save()
    ...
    def signup(request):
        if request.method == 'POST':
            ...
            if user_password1 == user_password2:
                if User.objects.filter(Q(username__iexact=user_email) | Q(email__iexact=user_email)).exists():
                    messages.success(
                        request, "User Name Or Email  alredy registerd")
                    return redirect("/signup")
                else:
                    user = User.objects.create_user(
                        username=user_email, first_name=first_name, last_name=last_name, email=user_email,
                        password=user_password2)
                    user.save()
                    subject = 'welcome to Active Sastre'
                    message = f'Hi {first_name}, thank you for registering in Active Sastre.'
                    email_from = settings.EMAIL_HOST_USER
...
class address:
    ...
    def SaveAddress(request):
        if request.method == 'POST':
            ...
            addresss_info = Address(user=user_pro, name=name, contact_no=mobile, pin_code=pin, state=state, city=city, house_no=house_no, area_landmark=area)
            addresss_info.save()
...
class size:
    ...
    def addsize(request):
        if request.method == 'POST':
            ...
            if request.POST['femail'] == 'female':
                size_obj = Femail_size_Chart(
                    user=user_obj, name=request.POST['name'], gender=request.POST['femail'],  gown_length=request.POST['gown_length'], upper_chest=request.POST['upper_Chest'], chest=request.POST['chest'], waist=request.POST['waist'], upper_waist=request.POST['upper_waist'], stomach=request.POST['stomach'], hips=request.POST['hips'], shoulder=request.POST['shoulder'], front_neck_depth=request.POST['front_neck_depth'], back_neck_depth=request.POST['back_neck_depth'], neck_to_solider=request.POST['neck_to_solider'], sleeve_length=request.POST['sleeve_length'], sleev_around=request.POST['sleeve_around'], around_bust=request.POST['around_bust'], blouse_length=request.POST['blouse_length'], arm_hole=request.POST['arm_hole'], waist_to_ankel_length=request.POST['waist_to_ankel_length'], full_body_lenght=request.POST['full_body_lenght'], dress_length=request.POST['dress_length'], neck_line=request.POST['neck_line'], upper_bust=request.POST['upper_bust'], chest_bust=request.POST['chest_bust'], thigh=request.POST['thigh'], knee=request.POST['knee'], calf=request.POST['calf'], ankel_hem=request.POST['ankel_hem'], shoulder_full_lenght=request.POST['shoulder_full_lenght'])
                size_obj.save()
                messages.info(request, "Female Size is added Successfully!")
            else:
                size_obj_mail = MailSize(user=user_obj, gender=request.POST['femail'],  name=request.POST['name'], p_length=request.POST['p_Length'], p_Waist=request.POST['p_Waist'], p_Hips=request.POST['p_Hips'], p_Thigh=request.POST['p_Thigh'], p_Knee=request.POST['p_Knee'], p_Leg_Opening=request.POST['p_LegOpening'], p_Crotch_Or_Rise=request.POST['p_Crotch'], p_In_Seam=request.POST['p_Seam'], s_Shirt_lenght=request.POST['s_ShirtLen'], s_Sleeve_Length=request.POST[
                    's_Sleeve'], s_Shoulders=request.POST['s_Shoulders'], s_Chest=request.POST['s_Chest'], s_Overarm=request.POST['s_Overarm'], s_Waistcoat_Length=request.POST['Waistcoat'], s_Bicep_Loose=request.POST['s_Bicep'], s_Front_Chest=request.POST['s_FrontChest'], s_Front_Stomach=request.POST['s_Stomach'], s_Front_Hips=request.POST['s_Hips'], s_Wrist=request.POST['s_Wrist'], s_Neck=request.POST['s_Neck'])
                size_obj_mail.save()
...
def your_order(request):
    ...
    for detail in order_obj:
        orders.append({'prod_detail': Product_detail.objects.filter(
            id=detail.Product_Id)[0], 'order_obj1': detail})
    send_detail = {
        'order_obj': orders,
        'rateing_obj': rateing_obj
    }
...
def rate_product(request, p_id, or_id):
    order_obj = Oreder_Detail.objects.filter(
        user_id=request.user.id, Product_Id=p_id, id=or_id)[0]
    if order_obj.review == False:
        if request.method == 'POST':
            ...
            feedback_obj = user_feedback(
                stiching_quality=stiching, order=order_obj, user_id=request.user.id, product=Product_detail.objects.filter(id=p_id)[0],  product_quality=product_quality, fitting_quality=fitting, liked=like_product, message=review)
            feedback_obj.save()
